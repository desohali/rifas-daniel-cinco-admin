...
const ParGuarani = () => {
  ...
  const [listaDeBoletosConPremioParGuarani, setListaDeBoletosConPremioParGuarani] = React.useState([]);
  ...
  const BOLETOS_CON_PREMIO_RANDOM = listaDeBoletosConPremioParGuarani.map((b: any) => ({
    ...b,
    premios: [OPTIONS_PREMIOS.find(({ _id }: any) => b.premioParGuarani == _id)]
  }));
  ...
  const {
    ...
    refetch
  } = useListarBoletosQueryQuery({ _idRifa: rifaDetalles?._id, boletosGanadoresParGuarani: true });

  React.useEffect(() => {
    if (dataBoletosGanadores) {
      setListaDeBoletosConPremioParGuarani(dataBoletosGanadores);
    }
      ...
      <Form
        ...
        onFinish={async (values) => {
          formPremios.validateFields().then((valuesPremios) => {
            if (!valuesPremios?.items || valuesPremios?.items?.length === 0 || valuesPremios?.items?.length < 2) {
              message.error("Debe agregar al menos 2 premios");
              return;
            }

            if (rifaDetalles?._id && rifaDetalles?.premiosYCantidadParGuarani?.length > 0) {
              message.error("No se puede editar una rifa con premios [par guaraní] asignados");
              return;
            }
            ...
            registrarRifa({
              ...
              premiosYCantidadParGuarani: valuesPremios?.items || [],
            }).then(() => {
        ...
        <Row gutter={16}>
          ...
          <Col xs={24} sm={24} md={24} lg={24}>
            <FormPremiosDinamicos
              titulo="Premios par guarani"
              form={formPremios}
              optionsPremios={OPTIONS_PREMIOS}
              monedaCarpeta={MONEDA_CARPETA?.simbolo}
              initialValues={rifaDetalles?.premiosYCantidadParGuarani || []} />
          </Col>
        </Row>
        <Row gutter={16}>
          <Col span={24}>
      ...
      <Row gutter={16}>
        <Col span={24}>
          <List
            ...
            header={(
              <div>
                <h4>{`Lista de ganadores par guarani : ${listaDeBoletosConPremioParGuarani.length}`}</h4>

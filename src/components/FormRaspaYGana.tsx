...
const FormRaspaYGana = () => {
  ...
  const BOLETOS_CON_PREMIO_RANDOM = listaDeBoletosConPremioRandom.map((b: any) => ({
    ...b,
    premios: [OPTIONS_PREMIOS.find(({ _id }: any) => b.premioRaspaYGana == _id)]
  }));
  ...
  const {
    ...
    refetch
  } = useListarBoletosQueryQuery({ _idRifa: rifaDetalles?._id, boletosGanadoresRaspaYGana: true });
      ...
      <Form
        ...
        onFinish={async (values) => {
          formPremios.validateFields().then((valuesPremios) => {
            ...
            if (rifaDetalles?._id && rifaDetalles?.premiosYCantidadRaspa?.length > 0) {
              message.error("No se puede editar una rifa con premios [raspa y gana] asignados");
              return;
            }
        ...
        <Row gutter={16}>
          ...
          <Col xs={24} sm={24} md={24} lg={24}>
            <FormPremiosDinamicos
              titulo="Premios raspa y gana"
              form={formPremios}
              optionsPremios={OPTIONS_PREMIOS}
              monedaCarpeta={MONEDA_CARPETA?.simbolo}
              initialValues={rifaDetalles?.premiosYCantidadRaspa || []} />
          </Col>
        </Row>
        <Row gutter={16}>
          <Col span={24}>
      ...
      <Row gutter={16}>
        <Col span={24}>
          <List
            ...
            header={(
              <div>
                <h4>{`Lista de ganadores raspa y gana : ${listaDeBoletosConPremioRandom.length}`}</h4>
